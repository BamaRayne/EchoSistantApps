...
metadata {
	...
	tiles (scale:1) {
        ...
        standardTile("lock", "device.lock", inactiveLabel: false, height:2, width:2, canChangeIcon: false) {
            state "default", label: "lock\nnot used"
            state "unlocked", label:'${name}', backgroundColor: "#ffffff"//, icon:"st.lock.lock.unlocked" 
            state "locked", label:'${name}', backgroundColor: "#53a7c0"//, icon:"st.lock.lock.locked" 
        }
...
def syncDevices(cmd){
	...
	if (inLockOn in ["0","1"]){
		if (inLockOn == cmd) sendEvent(name: "lock", value: "locked")
        else sendEvent(name: "lock", value: "unlocked")				
    } else sendEvent(name: "lock", value: null, displayed	: false)
    if (luxOn in ["0","1"]){
    	if (luxOn == cmd){
...
def localOn() {
	if (device.currentValue("locked") != "on"){
...
def localOff() {
	if (device.currentValue("unlocked") != "off"){
